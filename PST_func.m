...
MMT=complex(MMT);
for k=1:size(MMT,2)
    H=sin(pi.*MMT(1,k).*R).*exp(i*MMT(2,k).*theta);
    ...
    %             if idx(s,t)~=0
    %                 f=img2(s,t); % 像素灰度值
    %                 H=sin(pi*MMT(1,k)*r(s,t)*r(s,t))*exp(i*MMT(2,k)*theta(s,t));
...
% for s=1:N
%     for t=1:N
%         H=sin(pi*MMT(1,:)*R(s,t)).*exp(i*MMT(2,:)*theta(s,t));
